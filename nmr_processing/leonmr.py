...
def NMR1D(
    ...
    normalise: switch to normalise data or not (default = False). British
               spelling, I'm foreign.
    showplot: switch for showing plot (True) or just load x, y data (False)
              (default = True)
...
def NMR2D(
    ...
    if XDIM_F1 == 1:
        ...
        # Split into submatrices
        blocks = np.array(np.array_split(column_matrix, submatrix_number))
        # Reshape these submatrices so each has its own 1D array
        blocks = np.reshape(blocks, (submatrix_rows, submatrix_cols, -1))
        # Concatenate submatrices in the correct orientation
        real_spectrum_2d = np.vstack(
            [np.hstack([np.reshape(c, (XDIM_F1, XDIM_F2)) for c in b]) for b in blocks]
        )
...
def find_gamma(isotope):
    ...
    if df["Nuclide"].isin([isotope]).any():
        ...
        print(
            "Isotope string not recognised, please input a string of format e.g.,"
            "'1H' or '27Al' and ensure it is an NMR active nuclide."
        )
        return

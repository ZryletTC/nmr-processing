"""
Helpful utilities for working with NMR data.
"""

import pandas as pd

# List of all spin-1/2 nuclei
SPIN_HALF_LIST = [
        "1H",
        "3H",
        "3He",
...
def nucleus_label(nucleus):
    ...
    for index, char in enumerate(nucleus):
        if char.isalpha():
            number = nucleus[:index]
            symbol = nucleus[index:]
            break

    if nucleus in SPIN_HALF_LIST:
        labeltext = (
            r"$\mathregular{^{" + number + "}}$" + symbol + " chemical shift / ppm"
        )
    else:
        labeltext = r"$\mathregular{^{" + number + "}}$" + symbol + " shift / ppm"
    return labeltext
...
def find_gamma(nuclide):
    ...
    found = df["Nuclide"] == nuclide
    if any(found):
        gamma = df.loc[found]["Gyromagnetic Ratio (MHz/T)"]
        gamma = float(gamma.iloc[0]) * 1e6

        return gamma
    raise ValueError(
            "Nuclide string not recognised, please input a string of format e.g.,"
